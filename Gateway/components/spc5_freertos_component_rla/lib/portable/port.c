...
/* Multi-core system configured in AMP mode. */
volatile uint32_t *systime;

#define OSAL_SYSTIME_SETUP()     do { systime = osalSysTimeSetup(configTICK_RATE_HZ); } while (FALSE)
#define OSAL_SYSTIME_INC()       do { (*systime)++; } while (FALSE)

#else /* OSAL_CORE_CONFIG == OSAL_SINGLE_CORE */
/* Single-core system. */

#define OSAL_SYSTIME_SETUP()     do { (void)osalSysTimeSetup(configTICK_RATE_HZ); } while (FALSE)
#define OSAL_SYSTIME_INC()
#endif /* OSAL_CORE_CONFIG == OSAL_MULTI_CORE */
